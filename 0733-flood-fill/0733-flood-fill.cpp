...
public:
	...
	int initialcolor;
	void dfs(vector<vector<int>>&image,int sr,int sc,int color,vector<vector<int>>&visit){
		visit[sr][sc]=1;
		image[sr][sc]=color;
		int n=sz(image);
		int m=sz(image[0]);
		for(int i=0;i<4;i++){
			int nx=X[i]+sr;
			int my=Y[i]+sc;
			if(valid(nx,my,n,m,visit)&& image[nx][my]==initialcolor){
				dfs(image,nx,my,color,visit);
			}
		}
