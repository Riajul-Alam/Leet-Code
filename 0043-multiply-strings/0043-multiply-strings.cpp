    ...
    string multiply(string num1, string num2) {
        ...
        for(int i=m-1;i>=0;i--){
          ...
          for(int j=n-1;j>=0;j--){
            ...
            ll rem=z%10,d=z/10;
            //cerr<<char(rem+'0');
            a[i][ind]=char(rem+'0');carry=d; 
            //debug(a[i][ind])
            ind--;
          }
          if(carry){
            a[i][ind]=char(carry+'0');
          }//cerr<<endl;
          c1--;
        }
        string final;
        for(int i=1;i<m;i++){
          ...
          for(int j=n+m;j>=0;j--){
            int sum=0;
            sum=(a[i-1][j]-'0')+(a[i][j]-'0')+carry;
            a[i][j]=char((sum%10)+'0');cerr<<a[i][j];
            carry=sum/10;
          }cerr<<endl;
          if(carry){
            for(int k=0;k<=n+m;k++){
              if(a[i][k]){
                a[i][k-1]=char(carry+'0');break;
              }
            }
          }
        }
