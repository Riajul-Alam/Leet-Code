...
class Solution {
public:
    int mxProduct(vector<int>&a){
      int n=sz(a);int ans=a[0];int minus=0;
      if(a[0]<0)minus++;
      for(int i=1;i<n;i++){ans*=a[i];if(a[i]<0){minus++;}}
      if(minus%2==0)return ans;
      if(n==1){
        return a[0];
      }
    ...
    int maxProduct(vector<int>& nums) {
      int n,m,k,x,y,z,sum=0,c1=0,c2=0,ans=-1e9;
      n=nums.size();vector<int>a=nums,b;
      for(int i=0;i<n;i++){
        if(a[i]==0 && sz(b)){
          c1++;
          debug(b)
          ans=max(ans,mxProduct(b));b.clear();
        }else{
          if(a[i])b.pb(a[i]);
        }
      }
