...
public:
    int maximumBags(vector<int>& capacity, vector<int>& rocks, int additionalRocks){
      int n,m,k,x,y,z,sum=0,c1=0,c2=0,ans=0;m=additionalRocks;
      n=capacity.size();vector<int>a=capacity;
      vector<pair<int,int>>need(n,{0,0});
      f0(n){
        need[i]={capacity[i]-rocks[i],i};
      }
      sort(all(need));
      f0(n){
        x=need[i].ff;
