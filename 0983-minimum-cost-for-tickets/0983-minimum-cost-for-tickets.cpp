...
class Solution {
public:
    vector<int>tmp{1,7,30};
    int f(int i,int cur,vector<int>&a,vi&c,vvi &dp){
        if(i>=sz(a)){
            return 0;
        }
        if(dp[i][cur]!=-1)return dp[i][cur];
        int ans=INF,ans1=0;dp[i][cur]=INF;
        if(cur>=a[i]){
            ans=f(i+1,cur,a,c,dp);
        }
        for(int j=0;j<3;j++){
            ans=min(ans,c[j]+f(i+1,a[i]+tmp[j]-1,a,c,dp));
        }
        return dp[i][cur]=ans;
    }
    int mincostTickets(vector<int>& a, vector<int>& c) {
        vector<vector<int>>dp(400,vector<int>(400,-1));
        int ans=f(0,0,a,c,dp);
        return ans;
    }
};
