...
public:
	int final=0;
	pair<int,char> dfs(int node,vvi &g,vector<bool>&vis,string &s){
		vis[node]=1;
		int mx=0,mx2=0;
		char mxc;
		for(auto v:g[node]){
			if(!vis[v]){
				pair<int,char>tm=dfs(v,g,vis,s);
				if(tm.ss!=s[node]){
					if(tm.ff+1>=mx){
						mx2=mx;
						mx=tm.ff+1;
    ...
    int longestPath(vector<int>& parent, string s) {
        ...
        pair<int,char> ans=dfs(leaf,g,vis,s);
        if(ans.ss!=s[leaf]){
        	final=max(final,ans.ff+1);
        }
        else final=max(final,ans.ff);
        return final+1;
    }
};
