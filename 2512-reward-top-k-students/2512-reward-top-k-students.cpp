...
public:
  vector<int> topStudents(vector<string>& positive_feedback, vector<string>& negative_feedback, vector<string>&report, vector<int>& student_id, int k) {
    int n, m, x, y, z, sum = 0, c1 = 0, c2 = 0, ans = 0;
    map<int, vector<string>>mp;
    map<string, int>pr, nr;
    f0(sz(positive_feedback)) {
      pr[positive_feedback[i]] = 1;
    }
    f0(sz(negative_feedback)) {
      nr[negative_feedback[i]] = 1;
    }
    for (int i = 0; i < sz(report); i++) {
      mp[student_id[i]].pb(report[i]);
    }
