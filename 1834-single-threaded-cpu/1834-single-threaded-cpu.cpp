...
public:
    vector<int> getOrder(vector<vector<int>>& tasks) {
        ...
        int n=tasks.size();
        int ind=0,mn=1000000005,mn1=1000000005;
        f0(n){
          a.pb({{tasks[i][0],tasks[i][1]},i});
          if(tasks[i][0]<mn){
            ind=i;mn=tasks[i][0];
            mn1=tasks[i][1];
          }
          else if(tasks[i][0]==mn && tasks[i][1]<mn1){
            ind=i;mn=tasks[i][0];
            mn1=tasks[i][1];
          }
        }
        sort(all(a));
        vector<int>result;result.pb(ind);
        ll time=tasks[ind][0]+tasks[ind][1];
        int j=0;
        while(j<n || !ms.empty()){
            while(j<n){
                if(time<a[j].ff.ff){
                ...
                if(a[j].ss!=ind)ms.insert({a[j].ff.ss,a[j].ss});
                j++;
            }
            if(ms.empty()&& j>=n){
                break;
            ...
            if(ms.empty() && j<n){
                while(a[j].ss==ind && j<n){
                    j++;
                }
                if(j<n){
                    if(time<a[j].ff.ff)time=a[j].ff.ff;
                    ms.insert({a[j].ff.ss,a[j].ss});j++;
                }
            }
            if(ms.size()>0){
                auto q=ms.begin();
                time+=(q->ff);
                result.push_back(q->ss);
                ms.erase(q);
            }
        }
        return result;
