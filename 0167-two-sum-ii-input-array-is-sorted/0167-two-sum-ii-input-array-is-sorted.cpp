...
public:
    vector<int> twoSum(vector<int>& numbers, int target) {
        ...
        for(int i=0;i<numbers.size();i++){
            m[numbers[i]]++;
            mp[numbers[i]]=i+1;
            if(vp[numbers[i]].size()<2)vp[numbers[i]].push_back(i+1);
        }
        for(auto q:m)a.push_back(q.first);
        vector<int>ans;
        int c1=0,n=a.size();
        for(int i=0;i<n;i++){
            for(int j=i+1;j<n;j++){
                if(a[i]+a[j]==target){
                    ...
                    ans.push_back(mp[a[j]]);c1++;break;
                }
            }
            if(c1)break;
        }
        for(auto q:vp){
            if(q.second.size()<=1)continue;
            int x,y;
            if(numbers[(vp[q.first][0])-1]+numbers[(vp[q.first][1])-1]==target){
                ans.push_back(vp[q.first][0]);
                ans.push_back(vp[q.first][1]);break;
            }
        }
        return ans;
    }
