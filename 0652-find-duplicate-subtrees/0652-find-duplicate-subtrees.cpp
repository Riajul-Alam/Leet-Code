...
class Solution {
public:
    string f(TreeNode * node,unordered_map<string,int>&subtree,vector<TreeNode*>&duplicate){
        if(!node)return "#";
        string left,right,s;
        left=f(node->left,subtree,duplicate);
        right=f(node->right,subtree,duplicate);
        s=left+","+right+","+tostr(node->val);
        if(subtree[s]==1)duplicate.pb(node);
        subtree[s]++;
        return s;
    }
